...
export default function CourseDetailPage() {
    ...
    const slug = params.slug as string // Using slug param
    ...
    const priceDisplay = course.price?.display || formatCurrency(course.price?.current_price || 0)
    const isFreeCourse = course.price?.is_free || course.price?.current_price === 0

    // Format rating display
    const hasRating = course.rating?.has_enough_ratings && course.rating?.average
    const ratingDisplay = hasRating ? course.rating.average?.toFixed(1) || "Chưa có đánh giá" : "Chưa có đánh giá"
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    <div className="flex justify-between">
                                        ...
                                        <span>{course.created_at ? new Date(course.created_at!).toLocaleDateString('vi-VN') : 'Chưa có thông tin'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">Cập nhật:</span>
                                        <span>{course.updated_at ? new Date(course.updated_at!).toLocaleDateString('vi-VN') : 'Chưa có thông tin'}</span>
                                    </div>
                                    {course.published_at && (
                                        <div className="flex justify-between">
