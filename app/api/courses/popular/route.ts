...
// Mock popular courses data - sorted by student count
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryParams: PopularCoursesQueryParams = {
    ...
    // Apply filters to popular courses
    let filteredCourses = [...mockPopularCourses]
    ...
    if (queryParams.difficulty) {
      ...
      const vietnameseDifficulty = difficultyMap[queryParams.difficulty.toLowerCase()]
      if (vietnameseDifficulty) {
        filteredCourses = filteredCourses.filter(course => 
          course.stats.level.toLowerCase() === vietnameseDifficulty
        )
      }
    }
    ...
    const response: ApiPopularCoursesResponse = {
      success: true,
      message: "Danh sách khóa học phổ biến được tải thành công",
      data: {
    ...
    return NextResponse.json({
      success: false,
      message: "Có lỗi xảy ra khi tải danh sách khóa học phổ biến",
